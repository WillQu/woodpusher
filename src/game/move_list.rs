...
pub fn generate_moves_one_square<'a>(
    game: &'a Game,
    player: Player,
    start_position: Position,
    directions: &[(i8, i8)],
...
fn generate_moves_with_limit<'a>(
    game: &'a Game,
    player: Player,
    start_position: Position,
    directions: &[(i8, i8)],
    ...
    for (x, y) in directions {
        ...
        for _ in 0..limit {
            let new_position = Position::from_u8(
                (position.column() as i8 + x) as u8,
                (position.row() as i8 + y) as u8,
            );
